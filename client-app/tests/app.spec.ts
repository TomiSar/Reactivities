...
// test('User can move to login page and login successfully', async ({ page }) => {
//     await page.goto('/');
//     await expect(page).toHaveTitle(/Reactivities/); // Tai "Reactivities"

//     // 1. Klikataan Login-nappia etusivulla
//     await page.getByRole('button', { name: /Login/i }).click();
//     await expect(page.getByRole('heading', { name: 'Login to Reactivities', level: 2 })).toBeVisible();

//     // 2. Varmistetaan, että ollaan kirjautumislomakkeella (URL muuttuu)
//     await expect(page).toHaveURL(/.*localhost:3000/);

//     // 3. Täytetään testitunnukset (vaihda nämä omiin testi-tunnuksiisi)
//     await page.getByPlaceholder('Email').fill('bob@test.com');
//     await page.getByPlaceholder('Password').fill('Pa$$w0rd');
//     await page.locator('form').getByRole('button', { name: 'Login' }).click();
...
// test('User login with invalid credentials returns error', async ({ page }) => {
//     await page.goto('/');
//     await expect(page).toHaveTitle(/Reactivities/);
...
// test('User can register and go to activities', async ({ page }) => {
//     const randomUser = createRandomUser();
//     const userLower = randomUser.toLowerCase();
...
//     await expect(page).toHaveTitle(/Reactivities/);
//     await page.getByRole('button', { name: /Register/i }).click();
//     await expect(page.getByRole('heading', { name: 'Sign up to Reactivities', level: 2 })).toBeVisible();

//     await page.getByPlaceholder('Display Name').fill(randomUser);
//     await page.getByPlaceholder('Username').fill(userLower);
//     await page.getByPlaceholder('Email').fill(`${userLower}@${userLower}.com`);
//     await page.getByPlaceholder('Password').fill('Pa$$w0rd');
